...
class 缓动:
	...
	func 动画(_参数: Dictionary) -> 缓动:
		...
		if is_instance_valid(物体):
			#print(属性,"--",属性 in 物体)
			#引擎.调试.打印("测试过度",过渡输入值,过渡样式)
			#引擎.调试.打印("测试缓动",缓动输入值,缓动样式)
			
			if 属性 in 物体:
				if 过渡样式==null and 缓动样式==null:
					原生补间.tween_property(物体, 属性, 目标, 时间)
				elif 缓动样式==null:
					原生补间.tween_property(物体, 属性, 目标, 时间).set_trans(过渡样式)
				else:
