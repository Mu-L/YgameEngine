...
#					print("收到[%s:%s]的消息：%s" %[客户端IP,客户端端口,文本数据])
	...
	func _API(客户端,文本数据):
		var 请求数据=引擎.字符串.Json_到数据(文本数据)
		if 请求数据==null:
			return {"类型":"异常","数据":"异常js","状态":"失败","Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
		if 请求数据!=null:
			if ("类型" in 请求数据)==false:#print("无类型")
				return {"类型":"异常","数据":"异常LX","状态":"失败","Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
			if ("类型" in 请求数据)==true:
				##print("收到的类型:",请求数据.类型)
				##检测令牌
				if self.启动令牌==true:
					if "令牌" in 请求数据:
							...
							#print("这个令牌有,通过")
						if (请求数据.令牌 in self.令牌表["令牌表"])==false:
							return {"类型":"异常","数据":"异常数据LP","状态":"失败","Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
							
				#
				if has_method("__"+请求数据.类型):
					return call("__"+请求数据.类型,请求数据)
		pass
	func __账号注册(请求数据):#类型 账号 密码 账号限制字符
		...
		return {"类型": "账号注册","状态": 返回状态,"数据": 返回文本,"Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
	...
	func __账号登入(请求数据):#类型 账号 密码 
		...
		if 是否存在账号==true:
			if 引擎.加解密.字符串解密(self.数据_账号表[请求数据.账号]["密码"])==请求数据.密码:
				...
				#
				return {"类型": "账号登入","状态": 返回状态,"数据": 返回文本,"令牌":申请令牌码,"Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
			if self.数据_账号表[请求数据.账号]["黑名单"]=="1":
				返回文本 = "此账号已被封号";
				返回状态 = "失败";
		return {"类型": "账号登入","状态": 返回状态,"数据": 返回文本,"Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
	func __申请令牌():
	...
	func __临时存档申请(请求数据):
		...
		# 返回结果给客户端
		return {"类型": "临时存档申请","状态": "成功","数据": 兑换码,"Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
	func __临时存档兑换(请求数据):
		var 兑换码 = 请求数据.数据
		if (兑换码 in self.临时存档数据库)==false:
			return {"类型": "临时存档兑换","状态": "失败","数据": "","Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
		if (兑换码 in self.临时存档数据库)==true:
			var _数据 = self.临时存档数据库[兑换码]
			self.临时存档数据库.erase(兑换码)
			return {"类型": "临时存档兑换","状态": "成功","数据": _数据,"Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
	...
	func __聊天请求(请求数据):
		...
		#返回个人
		return {"类型":"聊天请求","状态":"成功","数据":"发送成功","Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
		pass
	func __聊天获取(请求数据):
		var 标记 = 请求数据.标记
		# 确保频道存在（即使是空的）
		...
		return {"类型": "聊天获取","状态": "成功","标记": 标记,"数据": 返回的记录,"Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
	func __PING(请求数据):
		return {"类型":"PING","状态":"成功","Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
	func __时间戳(请求数据):
		return {"类型":"时间戳","状态":"成功","数据":引擎.时间.取当前时间戳(),"Qid": 请求数据.Qid if "Qid" in 请求数据 else ""}
		pass
	func __伪随机():
		pass
...
class 弱网客户端:
	...
	var 同步回调字典 = {}  # 新增：用于存储每个请求的回调函数,发送数据 使用 Qid
	func __心跳包():
		await 引擎.场景.等待(200)
		var 数据={"类型":"PING"}
	...
	func 发送数据(数据:Dictionary,编码或回调:Variant="utf8"):#, 回调: Callable = Callable()):
		if self.令牌!="":
			数据.令牌=self.令牌
		
		# 新增：为每个请求生成唯一标识符
		if 编码或回调 is Callable:
			var Qid = 引擎.字符串.获取不重复随机码()
			数据.Qid = Qid
			self.同步回调字典[Qid] = 编码或回调
		
		self.客户端.UDP_发送数据(数据,"utf8")#发送
				...
				if "客户端收到消息" in 回调字典 and 回调字典["客户端收到消息"] is Callable:
					...
					if 数据 is Dictionary:
						...
						if "Qid" in 数据 :
							if 数据["Qid"]=="":
								回调字典["客户端收到消息"].call(数据)
							if 数据.Qid in self.同步回调字典:
								self.同步回调字典[数据.Qid].call(数据)
								self.同步回调字典.erase(数据.Qid)	
						else:
							回调字典["客户端收到消息"].call(数据)
						if 数据.类型=="账号登入":
							if 数据.状态=="成功":
