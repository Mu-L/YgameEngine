...
class 生命魔法:
	...
	func 设置上限生命值(值: float) -> void:
		var 有效上限 = max(float(值), 0.0)
		_上限生命值 = 引擎.加解密.浮点数加密(有效上限)
		# 若当前值超过新上限，自动截断
		if 获取当前生命值() > 有效上限:
			设置当前生命值(有效上限)
	...
	func 设置上限魔法值(值: float) -> void:
		var 有效上限 = max(float(值), 0.0)
		_上限魔法值 = 引擎.加解密.浮点数加密(有效上限)
		# 若当前值超过新上限，自动截断
		if 获取当前魔法值() > 有效上限:
			设置当前魔法值(有效上限)
	...
	# 战斗快捷操作（增减值封装）
	# ==============================================
	func 回血(恢复值: float) -> void:
		设置当前生命值(获取当前生命值() + 恢复值)
	
	func 扣血(伤害值: float) -> void:
		设置当前生命值(获取当前生命值() - 伤害值)
	
	func 回蓝(恢复值: float) -> void:
		设置当前魔法值(获取当前魔法值() + 恢复值)
	
	func 扣蓝(消耗值: float) -> void:
		设置当前魔法值(获取当前魔法值() - 消耗值)
