...
class 弱网服务端:
	...
	func ___计时器():#这是内部自动存档计时器
		var 储存文件=引擎.字符串.Json_到字符串(self.数据_账号)
		print("每10秒储存")
	...
	func ___读档():#首次读档使用
		if 引擎.文件.是否存在(引擎.文件.获取当前运行目录()+"/数据账号.json"):
			...
			self.数据_账号=引擎.字符串.Json_到数据(数据文本)
			print("发现文件数据,数据读取成功")
			self.___计时器()
		else:
			self.___计时器()
	...
	func _API(客户端,文本数据):
		...
		if 收到的数据==null:
			print("收到的json解析失败")
		if 收到的数据!=null:
			if ("类型" in 收到的数据)==false:
				print("无类型")
			if ("类型" in 收到的数据)==true:
				print("收到的类型:",收到的数据.类型)
				return call("__"+收到的数据.类型,收到的数据)
		pass
	func __账号注册(数据):#类型 账号 密码 账号限制字符
		print("准备注册",数据)
		var 返回状态 = "成功";
		var 返回文本 = "注册成功";
		var 特殊符号=false
		if 数据.账号限制字符==true:
			特殊符号=引擎.字符串.正则匹配(数据.账号,"^[A-Za-z0-9][A-Za-z0-9_]*$")==""
		#禁止注册 #未开放注册
		#检查特殊符号
		if 特殊符号==true:
			返回状态="失败"
			返回文本="账号不允许特殊符号注册"
		#检查重复账号
		if (数据.账号 in self.数据_账号)==true:
			返回状态="失败"
			返回文本="注册失败,账号已重复"
		#写入账号
		if 返回状态=="成功":
			self.数据_账号[数据.账号]={}
			self.数据_账号[数据.账号]={"账号":数据.账号,"密码":数据.密码,"黑名单":"0"}			
...
class 弱网客户端:
	var 客户端: 引擎.网络类.UDP服务端
	
	func 连接(ip="127.0.0.1",端口=25555):
		var UDP客户端=引擎.网络.UDP_创建客户端()
		UDP客户端.UDP_连接(ip,端口)
		pass
	pass
