...
class 等级经验系统:
	...
	# 获取当前累计经验（解密后转为整数）
	func 获取累计经验值() -> float:
		return 引擎.加解密.浮点数解密(_加密累计经验)
	
	# 设置总经验（转为浮点数加密存储）
	func 设置累计经验值(值: float,触发升级奖励:bool=false) -> void:
		var 有效经验 = max(0, 值)  # 经验不能为负
		_加密累计经验 = 引擎.加解密.浮点数加密(有效经验)
		_同步等级(触发升级奖励)  # 同步等级
	
	# 增加经验（整数增加值）
	func 经验值增加(增加值: float) -> void:
		if 增加值 <= 0:
			return
		设置累计经验值(获取累计经验值() + 增加值,true)
	func 经验值减少(减少值: float, 允许降级: bool = false) -> void:
		if 减少值 <= 0:
			return  # 减少值必须为正数
		
		var 当前总经验 = 获取累计经验值()
		var 当前等级内经验 = 获取当前等级已获经验值()  # 当前等级的已有经验（不包含上一级累计）
	...
	# 获取当前等级（解密后转为整数）
	func 获取当前等级() -> int:
		return int(引擎.加解密.浮点数解密(_加密当前等级))
	...
	func 获取当前等级已获经验值()->float:#计算当前等级经验量
		if _经验计算函数.is_null():
			return -1
		return 获取累计经验值()-_获取上一级的累计上限()
	func 获取当前等级所需经验值上限()->float:#计算当前等级总经验量
		if _经验计算函数.is_null():
			return -1
